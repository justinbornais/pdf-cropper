...
export async function generateFinalPDF({ pages, boxes, scale }) {
  ...
  for (let i = 0; i < boxes.length; i++) {
    ...
    fullCanvas.height = viewport.height * pages.length;
    ...
    if (contentAspect > pageAspect) {
      drawW = PAGE_WIDTH;
      drawH = PAGE_WIDTH / contentAspect;
    } else {
      drawH = PAGE_HEIGHT;
      drawW = PAGE_HEIGHT * contentAspect;
    }
    ...
    if (i > 0) pdf.addPage();
    pdf.addImage(imgData, "PNG", padX, padY, drawW, drawH);
  }
